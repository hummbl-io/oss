...
DENYFILE_PATTERNS: list[tuple[re.Pattern[str], str]] = [
    ...
    (re.compile(r"(?:^|[-_])AAR(?:[-_]|$)", re.I), "After Action Review (belongs in private repo)"),
    (re.compile(r"^RECEIPT[-_]", re.I), "internal receipt artifact"),
    (re.compile(r"^SESSION[-_]?(TRANSCRIPT|LOG)", re.I), "session transcript/log"),
    (re.compile(r"SESSION[_-]?FORENSICS", re.I), "session forensics/transcript (belongs in private repo)"),
    (re.compile(r"INLINE[_-]SESSION", re.I), "inline session review (belongs in private repo)"),
    ...
    (re.compile(r"^AUDIT[-_]?FINDINGS", re.I), "internal audit findings (belongs in private repo)"),
    (re.compile(r"PRODUCTION[-_]?AUDIT", re.I), "internal production audit (belongs in private repo)"),
