...
Per SYNTHESIS.md §3.3 design (`_internal/governance/basen-architecture/SYNTHESIS.md`).

Architecture
------------
- Stdio JSON-RPC 2.0 server (MCP protocol 2024-11-05).
- Stdlib-only (no `mcp` library — wire protocol implemented from scratch
  per HUMMBL MCP convention; mirrors `hummbl_mcp/mcp/server.py` pattern).
...
# Default canonical registry path (JSON form; YAML form can be added later).
# Resolve to the packaged registry by default so `python -m
# hummbl_mcp.basen_server` works from any checkout. Operators can still
# point at a deployment-specific registry with BASEN_REGISTRY_PATH.
_PACKAGE_ROOT = Path(__file__).resolve().parents[1]
_STATE_ROOT = _PACKAGE_ROOT / "_state"
DEFAULT_REGISTRY_JSON = Path(
    os.environ.get(
        "BASEN_REGISTRY_PATH",
        str(_PACKAGE_ROOT / "cognition" / "data" / "base120_registry.json"),
    )
)
...
# Default tuple log path. Resolve to the packaged `_state/governance/`
# directory so persistence works on any checkout. Operators can still
# point at a deployment-specific log via BASEN_TUPLE_LOG. Matches the
# canonical default used by `hummbl_mcp/services/basen_tuple.py`.
# Stage-3 fix-up for convergent P1 (same
# class as the registry-path default fixed in a043eeaa).
_TUPLE_LOG_PATH = Path(
    os.environ.get(
        "BASEN_TUPLE_LOG",
        str(_STATE_ROOT / "governance" / "tuples.jsonl"),
    )
)
...
def _append_tuple_jsonl(record: dict[str, Any]) -> bool:
    ...
    Cross-platform: tries the canonical `hummbl_mcp._basen_tuple.append_tuple`
    ...
    # Prefer canonical writer when importable (POSIX). On Windows this hits an
    # ImportError on `import fcntl` inside append_tuple and we fall through.
    try:
        try:
            from hummbl_governance.primitives import (  # type: ignore
                BaseNTuple,
                append_tuple,
            )
        except ImportError:
...
MCP_TOOLS: list[ToolDefinition] = [
    ...
    ToolDefinition(
        name="basen_apply",
        description=(
            "Apply an operator to an input. Tier 1 write (emits EVIDENCE tuple). "
...
# ---------------------------------------------------------------------------
# Uses remote Ollama nomic-embed-text via stdlib urllib. Embeddings
...
# Default embedding cache dir. Resolve to the packaged `_state/basen/`
# directory so the cache survives across checkouts of the same repo
# without requiring an env override. Operators can still point at a
# deployment-specific dir via BASEN_EMBED_CACHE_DIR. Stage-3 fix-up
# for convergent P1.
EMBED_CACHE_DIR = Path(
    os.environ.get(
        "BASEN_EMBED_CACHE_DIR",
        str(_STATE_ROOT / "basen"),
    )
)


def _embed_text(text: str) -> list[float] | None:
    """Embed text via remote Ollama nomic-embed-text. None on failure."""
    import urllib.error
...
def _tool_recommend(args: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(desc, str) or not desc.strip():
        raise JsonRpcError(INVALID_PARAMS, "problem_description must be a non-empty string")
    ...
    tokens = [t for t in "".join(c.lower() if c.isalnum() else " " for c in desc).split() if len(t) >= 4]
...
def _tool_apply(args: dict[str, Any]) -> dict[str, Any]:
    ...
    if mode not in ("advisory", "analytic", "empirical"):
        raise JsonRpcError(INVALID_PARAMS, f"mode must be one of advisory|analytic|empirical; got {mode!r}")
...
def _resource_read(uri: str) -> dict[str, Any]:
    ...
    if tail.startswith("operator/"):
        ...
        return {"contents": [{"uri": uri, "mimeType": "application/json", "text": json.dumps(op, indent=2)}]}
    ...
    if tail.startswith("family/"):
        ...
        return {"contents": [{"uri": uri, "mimeType": "application/json", "text": json.dumps(body, indent=2)}]}
...
MCP_PROMPTS: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "arguments": [
            {"name": "artifact_path", "description": "Path to the artifact under review.", "required": True},
        ],
    },
]
...
def _prompt_get(name: str, args: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "description": next((p["description"] for p in MCP_PROMPTS if p["name"] == name), name),
        "messages": [
