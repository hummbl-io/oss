...
class CapabilityAdmissionPolicy:
    ...
    def request_admission(
        ...
        logger.info(
            f"Admission decision for {capability}: {decision.status.value} - {decision.reason}"
        )
        return decision
    ...
    def _grant_capability(
        self, request: AdmissionRequest, capability: Capability
    ) -> AdmissionDecision:
        """Grant a capability request"""
        grant_id = self._generate_id("grant")
        expires_at = (
            datetime.now(timezone.utc) + timedelta(minutes=self.default_grant_duration_minutes)
        ).isoformat()
    ...
    def approve_request(
        self, request_id: str, approved_by: str, expires_in_minutes: Optional[int] = None
    ) -> AdmissionDecision:
        """
        ...
        expires_at = (
            ...
            + timedelta(minutes=expires_in_minutes or self.default_grant_duration_minutes)
        ).isoformat()
...
async def main():
    ...
    if decision.status == AdmissionStatus.PENDING_APPROVAL:
        decision = policy.approve_request(
            request_id=decision.request_id, approved_by="operator", expires_in_minutes=30
        )
        ...
        is_valid, reason = policy.validate_grant(
            grant_id=decision.grant_id, capability="system.configure", action="modify"
        )
