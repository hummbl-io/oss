...
Usage:
    python3 bif_mcp_server.py

Configure in Claude Code settings.json:
    {
      "mcpServers": {
        "bif": {
          "command": "hummbl-mcp-bif"
        }
      }
    }
"""
...
SESSIONS_DIR = Path(os.environ.get("BIF_SESSIONS_DIR") or _default_sessions_dir())
...
QUALITY_CHECKS = [
    {
        "id": "metadata",
        "label": "Metadata header present (source URL, fetch date, purpose)",
    },
    {
        "id": "structured",
    ...
    {
        "id": "tables",
        "label": "Tables for comparisons (models, features, pricing, options)",
    },
    {
        "id": "no_stale",
    ...
    {
        "id": "delta",
        "label": "Delta noted -- what's new vs. what a reader might already know",
    },
    {
        "id": "actionable",
    ...
    {
        "id": "self_contained",
        "label": "Self-contained -- usable without opening external links",
    },
    {
        "id": "token_budget",
        "label": "Under context budget (target: under 50K tokens per file)",
    },
]
...
def load_templates():
    ...
    templates = {}
    templates_dir = next(
        (path for path in TEMPLATE_DIR_CANDIDATES if path.exists()), None
    )
...
def _check_token_budget(content):
    ...
    ok = tokens < 50000
    suffix = (
        " (under 50K limit)" if ok else " (OVER 50K limit -- split into sub-batches)"
    )
    return {
        "check": "token_budget",
        "passed": ok,
