...
ISO_CROSSWALK = {
    ...
    "ISO42001:5": {
        ...
        "soc2": "CC1",
        "description": "AI leadership and commitment",
    },
    "ISO42001:6": {
        "nist_ai_rmf": "MAP-1",
        "soc2": "CC3",
        "description": "AI planning and risk",
    },
    "ISO42001:7": {
        "nist_ai_rmf": "GOVERN-3",
        "soc2": "CC1",
        "description": "AI support and resources",
    },
    "ISO42001:8": {
...
def _build_coverage_map(controls_dict, tuple_mapping, entries):
    ...
    for subcat_id in [
        sc for func in controls_dict.values() for sc in func["subcategories"]
    ]:
        coverage[subcat_id] = []
...
def _nist_map_controls(governance_dir: str) -> dict:
    ...
    ai_rmf_coverage = _build_coverage_map(
        NIST_AI_RMF_CONTROLS, _TUPLE_TO_AI_RMF, entries
    )
...
def _iso_crosswalk(governance_dir: str) -> dict:
    ...
    for iso_ctrl, mapping in ISO_CROSSWALK.items():
        ...
        csf_count = (
            csf_controls.get(nist_csf_id, {}).get("evidence_count", 0)
            if nist_csf_id
            else 0
        )
        rmf_count = (
            ai_rmf_controls.get(nist_ai_rmf_id, {}).get("evidence_count", 0)
            if nist_ai_rmf_id
            else 0
        )
    ...
    return {
        ...
        "coverage": {
            ...
            "iso_coverage_pct": round(iso_covered / iso_total * 100, 1)
            if iso_total
            else 0,
...
def _compliance_evidence_export(governance_dir: str, framework: str) -> dict:
    ...
    if framework in ("owasp", "all"):
        evidence["owasp_agentic"] = _report_to_evidence(
            mapper.generate_owasp_report(days=90)
        )
                ...
                "interactions": {
                    ...
                    "items": {
                        ...
                        "properties": {
                            "source": {
                                "type": "string",
                                "description": "Source agent or component",
                            },
                            "target": {
                                "type": "string",
                            ...
                            "rate_limited": {
                                "type": "boolean",
                                "description": "Whether rate limiting is in place",
                            },
                        },
