...
def _extract_numeric_values(descriptions: list[str], pattern_keyword: str) -> list[float]:
    ...
    for desc in descriptions:
        if pattern_keyword.lower() in desc.lower():
            ...
            param_pattern = (
                rf"{re.escape(pattern_keyword)}\s*[=:]?\s*([-+]?\d*\.?\d+(?:e[-+]?\d+)?)"
            )
...
class TracePlanner:
    ...
    def _get_near_misses(self, threshold: float = 0.003) -> list[ExperimentRecord]:
        ...
        return [
            r
            for r in self.analyzer.records
            if r.outcome == "discard" and r.val_bpb > 0 and (r.val_bpb - best) <= threshold
        ]
    ...
    def _descriptions_for_category(self, category: str) -> list[str]:
        ...
        return [r.description for r in self.analyzer.records if r.category == category]
    ...
    def to_strategy_md(self, plan: ExperimentPlan) -> str:
        ...
        lines.append("---")
        lines.append(
            f"*Generated by HUMMBL TracePlanner — {len(plan.experiments)} experiments queued*"
        )
        return "\n".join(lines)
