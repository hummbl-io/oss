...
def wilson_score_interval(p: float, n: int, confidence_level: float = 0.95) -> UncertaintyQuantification:
    ...
    if n == 0:
        return UncertaintyQuantification(
            value=0.0, confidence_interval=(0.0, 0.0), confidence_level=confidence_level, sample_size=0, method="wilson"
        )
...
def bootstrap_test(
    values_a: List[float], values_b: List[float], n_bootstrap: int = 10000, confidence_level: float = 0.95
...
def calculate_effect_size(values_a: List[float], values_b: List[float]) -> Dict[str, float]:
...
def ks_test(values_a: List[float], values_b: List[float]) -> Dict[str, Union[float, bool, str]]:
    ...
    critical_values = {0.10: 1.224 / math.sqrt(n_eff), 0.05: 1.358 / math.sqrt(n_eff), 0.01: 1.628 / math.sqrt(n_eff)}
...
def wilson_score_ci(successes: int, n: int, confidence: float = 0.95) -> Dict[str, float]:
...
def binary_metric_ci(values: List[Union[bool, int, float]], confidence: float = 0.95) -> Dict[str, float]:
