...
- OAuth WG: [draft-ietf-oauth-identity-chaining-17](https://datatracker.ietf.org/doc/draft-ietf-oauth-identity-chaining/) (`iesg_state`: RFC Ed Queue; intended Proposed Standard)
- Proposed Standards (published June 2026): [RFC 9943](https://datatracker.ietf.org/doc/rfc9943/) SCITT, [RFC 9942](https://datatracker.ietf.org/doc/rfc9942/) COSE Receipts
**Effective date / timeline**: Internet-Drafts are working documents (BCP 78/79), not RFCs. RFC 9943 and RFC 9942 are Proposed Standards as of June 2026. Individual agent-delegation drafts are not WG-adopted as of 2026-08-31.
...
| draft-ietf-wimse-arch-08 | WIMSE architecture: workload identity across systems, not an agent-chain protocol | ⚪ Boundary: identity registry is in-process agent aliases/trust tiers, not WIMSE architecture | `hummbl_governance/identity.py` |
| draft-ietf-wimse-identifier-03 | Canonical workload identifier URI in a trust domain | 🟡 Partial: `AgentRegistry` stores string agent IDs; not WIMSE URI identifiers | `hummbl_governance/identity.py` |
| draft-ietf-wimse-workload-creds-02 | WIT credentials; algorithms used with symmetric keys MUST NOT be used | 🟡 Partial: HMAC-SHA256 tokens exist and are the shipped signing model; they are not WITs and contradict the symmetric-alg forbid | `hummbl_governance/delegation.py`, `hummbl_governance/identity.py` |
| draft-ietf-wimse-wpt-02 | Workload Proof Token for presenting workload credentials | ⚪ Boundary: no WPT issuer or verifier in this library | n/a — boundary row |
| draft-ietf-oauth-identity-chaining-17 | RFC Editor Queue, intended Proposed Standard: OAuth identity/authorization chaining across domains — not agent-chain tokens | ⚪ Boundary: different problem (cross-domain OAuth grants), not HUMMBL agent capability tokens | n/a — boundary row |
...
| RFC 9942 COSE Receipts | Proposed Standard (June 2026): COSE receipts proving VDS inclusion/consistency | 🟡 Partial: HMAC-SHA256 receipt signatures and hash-chain monitors exist; they are not COSE Receipts (`receipts` label 394) | `hummbl_governance/kernel/receipt_engine.py`, `hummbl_governance/kernel/receipt_integrity_monitor.py` |
...
5. **Receipts are HMAC JSON/TSV**, not COSE_Sign1 and not RFC 9942 receipts.
