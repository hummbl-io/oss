...
(`as_of` 2026-08-26). Honesty over completeness. A number that is not backed by
a public receipt is dropped or scoped, not promoted.
...
GAP-001, GAP-002, and GAP-003 remain **open**. This document does not close them.
...
| Production-tested / runs daily in production | not verified (GAP-001) | Landing claim LANDING-012: the linked evidence corpus does not include a production-use receipt. | Do not use a general production-tested claim. Keep Alpha. |
...
These gaps stay open. Closing them is out of scope for this documentation alignment.

| ID | Gap | Effect |
| -- | --- | ------ |
| GAP-001 | No production-use receipt is linked in the landing claims ledger. | Do not claim general production-tested status. |
...
- Production-use receipt if a general production-tested claim is made (GAP-001).
...
- Do not use production, customer, benchmark, extraction, or comprehension
  claims without a current public receipt.
...
| Landing claims ledger | 1.4.2 | 2,463 passed / 3 skipped | 34 | Canonical public promotion source (`as_of` 2026-08-26). |
